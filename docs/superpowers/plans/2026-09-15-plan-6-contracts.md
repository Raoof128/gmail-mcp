...
export const Authorization = z
  .object({
    ...
    expiresAt: Time,
    capabilities: z.array(z.enum(["send", "revoke", "power-loss", "deploy", "restore", "staging", "enable"])).max(7),
...
export const MutationSlot = z
  .object({
    ...
    capability: z.enum(["send", "revoke", "staging"]),
...
export const ManifestV2 = z
  .object({
    version: z.literal(2),
    purpose: z.enum(["recovery-mode", "release-component"]),
    phase: z.enum(["prepare", "probe", "run", "enable", "disable"]),
    target: Target,
    authorization: ArtifactRef,
    preparation: ArtifactRef,
    preparationCommitment: Hash,
    preparationRoot: Hash.nullable(),
    transition: ArtifactRef.nullable(),
    ...
    caseIds: z.array(z.enum(CaseIds)).min(1).max(11),
  ...
  .refine(
    (m) =>
      new Set(m.caseIds).size === m.caseIds.length &&
      new Set(m.probeIds).size === m.probeIds.length &&
      (m.purpose === "release-component"
        ? m.mode === null &&
          ...
          m.phase !== "enable"
        : m.mode !== null &&
          ((m.phase !== "run" && m.phase !== "enable") ||
            (m.expectedEpoch !== null && m.preparationRoot !== null && m.transition !== null))),
  );
...
export const RestoreTarget = z
  ...
  .strict()
  .refine((r) => r.databaseId === r.snapshot.databaseId);
...
export interface PreparationContext {
  ...
  workerRequest(sampleId: string, slotId: string | null, tool: string, args: Record<string, unknown>): Promise<unknown>;
  nativeRequest(
    sampleId: string,
    command: "login" | "logout" | "stage" | "save" | "receipt",
...
| resources           | Live serial ordinals 1–10 with maxPayload=true; one live concurrent ordinal 1 with concurrentStreams=2; coverageComplete=true; peakMemoryBytes<128000000 and cpuMs<routeCpuLimitMs from verified source, zero errors     |
...
First compute allocationHash as `identityHash("preparation", identity, canonicalize)`; intent templates reference this hash, avoiding a self-reference. The pre-execution commitment is `identityHash("preparation-commitment", {identity, intentRefs}, canonicalize)` with template refs in allocation/slot order and a verified ref for every slot. Enforce JSON depth <=8 and <=64 keys per object before recursive schema parsing, then the 65,536-byte file cap; oversized template bodies fail before credentials, even when a tool permits a larger body. PreparationOutcome references this committed sample, not a later selection of convenient operations. The preparation root function above verifies the closed allocation/outcome set; the full transition root is `identityHash("transition", {preparationCommitment, preparationRoot}, canonicalize)`; the preparation root already includes all terminal source hashes. A standalone call to the pure helper is not permission to probe.
