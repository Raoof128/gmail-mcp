...
## Runtime clarification: v2 command records (2026-09-16)

`preflight-v2.ts` defines `DeploymentReceiptV2` as version 2, a strict Snapshot, bundle SHA-256 and bounded platform script ETag. `PreparationCommitmentRecord` contains version 2, PreparationIdentity, the original authorization ArtifactRef and at most 64 ordered intent ArtifactRefs. The preparation commitment hashes `{ identity, intentRefs }` under the existing preparation-commitment domain. Enable may use a separate current authorization while retaining the original fixture authorization reference.

Local preflight validates hashes and relationships; a deployment-exclusion artifact requires a trusted live verifier before dispatch. When that verifier is unavailable, the CLI writes a version-2 `command-status` record with the command, manifest SHA-256, `not_run` result and bounded reason. This record is not a CaseReport and cannot supply qualification evidence. A v2 run wraps one CaseReport and its frozen RunIdentity with an exit code derived from the verdict. Production controller and administrative bindings remain pending.

## Runtime clarification: durable sample checkpoints (2026-09-16)

The host journal provides read-only `openIntentJournal` reconciliation and a `sealSample` extension to IntentStore. Sample source filenames use SHA-256 of canonical `[authorizationId, sampleId]`; atomic exclusive publication pins the first terminal source before its derived outcome. Reconciliation verifies the source's exact byte hash and commitment. If the process stopped before writing the outcome, the reader derives it from the pinned source without mutation. Corrupt or mismatched records refuse instead of appearing absent.

For components allocating no mutation slots, `slotJournalRoot` must equal SHA-256 of UTF-8 `gmail-mcp/plan6/v2/slot-journal` followed by one newline and canonical `[]`. An arbitrary hash is insufficient. The durable component finalizer currently supports only these empty journals; nonempty mutation journals remain unavailable until their full provenance resolver is implemented.

## Runtime result projection implementation (2026-09-17)

The local implementation uses `IntentResultProjection` in `scripts/qualification/intent-resolution.ts`: `{version:2, consumption, tool, targetHash, recordedAt, fields}`. `consumption` is the full immutable Consumption record. `fields` permits only `handle`, `download_id`, `action_id`, `message_id`, `thread_id` and `operation_id`, each validated as Id; at least one must be present. Unknown fields refuse. Trusted transport adapters extract these values from actual responses. A caller-authored projection does not establish provider truth.

`openIntentResults` stores one projection per authorization/slot in the existing host journal. Publication requires an identical consumed record under the same preparation. Reads recheck that binding and return the hash of the exact private bytes. Duplicate identical publication preserves the original artifact; a different projection refuses. Result publication cannot consume a slot or authorize a retry. A process exit after publication leaves the first result available for read-only reconciliation and dependent argument resolution.

The local dispatcher requires both the result journal and a trusted projection adapter to use this path. It resolves references in template order, checks exact source hashes and reconstructs the final argument digest before existing admission checks. It currently accepts primary-target dependencies only. Sacrificial-target dependencies and mutation-bearing preparation closure remain incomplete; the default production commands retain their deployment-exclusion refusal.
